...
/* ===== END RK SLICE PATCH V2 ===== */

#include "/tmp/rk_ue_la_policy.h"
#include "/tmp/rk_ue_la_policy.c"

#define RK_UE_LA_STATE_PATH "/opt/oai-rk/ue_la/ue_la.conf"
                       ...
                       const uint16_t rssi)
{
  ...
  const int default_target_snrx10 = gNB_mac->pusch_target_snrx10;
  const int target_snrx10 =
      rk_ue_la_get_pusch_target_snrx10(current_rnti, default_target_snrx10);
  ...
  if (UE) {
    ...
    if (rssi > 0) {
      ...
      LOG_I(NR_MAC,
            "RK-LA UL-TPC UE %04x pusch_target_snrx10=%d tpc0=%d pusch_snrx10=%d\n",
            UE->rnti,
            target_snrx10,
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  const int ul_now_tick = sched_frame * slots_per_frame + sched_slot;
  rk_ue_la_reload_if_needed(RK_UE_LA_STATE_PATH);

  const int min_rb_default = nrmac->min_grant_prb;
  ...
  UE_iterator(UE_list, UE) {
    ...
    const bool do_sched = nr_UE_is_to_be_scheduled(&nrmac->frame_structure,
                                                   ...
                                                   sched_frame,
                                                   sched_slot,
                                                   rk_ue_la_get_ulsch_max_frame_inactivity(UE->rnti, nrmac->ulsch_max_frame_inactivity));
    ...
    const int max_mcs = rk_ue_la_get_ul_max_mcs(UE->rnti, bo->max_mcs, max_mcs_table);
    const int eff_inact =
        rk_ue_la_get_ulsch_max_frame_inactivity(UE->rnti, nrmac->ulsch_max_frame_inactivity);
    const int eff_min_rb =
        rk_ue_la_get_min_grant_prb(UE->rnti, min_rb_default);
    const float eff_ul_sched_mul =
        rk_ue_la_get_ul_sched_mul(UE->rnti, 1.0f);
    const int eff_ul_maxcg =
        rk_ue_la_get_ul_maxcg_override(UE->rnti, -1);
    const int eff_ul_small_burst_bytes =
        rk_ue_la_get_ul_small_burst_bytes(UE->rnti, 0);
    const float eff_ul_small_burst_mul =
        rk_ue_la_get_ul_small_burst_mul(UE->rnti, 1.0f);

    LOG_I(NR_MAC,
          "RK-LA UL-CFG UE %04x ul_max_mcs=%d min_grant_prb=%d ulsch_max_frame_inactivity=%d ul_sched_mul=%.2f ul_maxcg_override=%d ul_small_burst_bytes=%d ul_small_burst_mul=%.2f\n",
          UE->rnti,
          max_mcs,
          eff_min_rb,
          eff_inact,
          eff_ul_sched_mul,
          eff_ul_maxcg,
          eff_ul_small_burst_bytes,
          eff_ul_small_burst_mul);
    int selected_mcs;
    ...
    LOG_D(NR_MAC, "[UE %04x][%4d.%2d] b %d, ul_thr_ue %f, tbs %d, coeff_ue %f, sched_inactive %d\n",
          ...
          coeff_ue,
          sched_inactive);
    const int ul_slice = rk_slice_assoc_get_ul_id(UE->rnti);
    const rk_slice_policy_t *ul_pol = rk_slice_policy_get(ul_slice);
    const float ul_mul = ul_pol->ul_weight_mul;
    const int ul_maxcg =
        (eff_ul_maxcg > 0) ? eff_ul_maxcg : ul_pol->ul_max_consecutive_grants;
    const int ul_throttled =
        rk_slice_runtime_ul_should_throttle(UE->rnti, ul_now_tick, ul_maxcg);

    const int ul_small_burst_hit =
        (eff_ul_small_burst_bytes > 0 && B > 0 && B <= eff_ul_small_burst_bytes);
    const float coeff_after_slice = coeff_ue * ul_mul;
    const float coeff_after_burst =
        coeff_after_slice * (ul_small_burst_hit ? eff_ul_small_burst_mul : 1.0f);
    const float final_coeff =
        ul_throttled ? 0.0f : (coeff_after_burst * eff_ul_sched_mul);

    UE_sched[numUE].coef = final_coeff;
    UE_sched[numUE].sched_inactive = sched_inactive;
    UE_sched[numUE].UE = UE;
    UE_sched[numUE].selected_mcs = selected_mcs;

    LOG_I(NR_MAC,
          "RK-SLICE UL-ORDER UE %04x ul_slice=%d ul_mul=%.2f coeff=%.4f final=%.4f\n",
          UE->rnti,
          ul_slice,
          ul_mul,
          coeff_ue,
    ...
    LOG_I(NR_MAC,
          "RK-LA UL-ORDER UE %04x ul_sched_mul=%.2f ul_small_burst_hit=%d ul_small_burst_mul=%.2f coeff_after_slice=%.4f coeff_after_burst=%.4f final=%.4f\n",
          UE->rnti,
          eff_ul_sched_mul,
          ul_small_burst_hit,
          eff_ul_small_burst_mul,
    ...
    if (ul_throttled) {
      LOG_I(NR_MAC,
            "RK-SLICE UL-LIMIT UE %04x ul_maxcg=%d throttled=1\n",
            UE->rnti,
            ul_maxcg);
    }
    numUE++;
  ...
  while (iterator->UE != NULL) {
    ...
    const int eff_min_rb =
        rk_ue_la_get_min_grant_prb(iterator->UE->rnti, min_rb_default);
    ...
    if (remainUEs[beam.idx] == 0 || n_rb_sched[beam.idx] < eff_min_rb) {
    ...
    uint16_t max_rbSize = iterator->sched_inactive ? eff_min_rb : bi.bwpSize;
    ...
    if (rbStart + eff_min_rb > bi.bwpSize || available_rb < eff_min_rb) {
      reset_beam_status(&nrmac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, dci_beam.new_beam);
      reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
    ...
    if((sched_ctrl->pcmax != 0 || sched_ctrl->ph != 0) && B > 0)
      nr_ue_max_mcs_min_rb(current_BWP->scs, sched_ctrl->ph, &sched, current_BWP, eff_min_rb, B, &available_rb, &sched.mcs);
    ...
    const int ul_slice_alloc = rk_slice_assoc_get_ul_id(iterator->UE->rnti);
    const rk_slice_policy_t *ul_pol_alloc = rk_slice_policy_get(ul_slice_alloc);
    const int ul_floor = ul_pol_alloc->ul_rb_floor;
    const int ul_cap = ul_pol_alloc->ul_rb_cap;

    if (ul_floor > 0 && sched.rbSize < ul_floor) {
      const int target_floor = (ul_floor < available_rb) ? ul_floor : available_rb;
      if (target_floor > sched.rbSize)
        sched.rbSize = target_floor;
    }

    if (ul_cap > 0 && sched.rbSize > ul_cap)
      sched.rbSize = ul_cap;
...
void post_process_ulsch(gNB_MAC_INST *nr_mac, post_process_pusch_t *pusch, NR_UE_info_t *UE, NR_sched_pusch_t *sched_pusch)
{
  ...
  const int ul_slice_log = rk_slice_assoc_get_ul_id(UE->rnti);
  const rk_slice_policy_t *ul_pol_log = rk_slice_policy_get(ul_slice_log);

  LOG_I(NR_MAC,
        "RK-SLICE UL-ALLOC UE %04x ul_slice=%d ul_cap=%d rbSize=%d current_rbs=%u\n",
        UE->rnti,
        ul_slice_log,
        ul_pol_log->ul_rb_cap,
        sched_pusch->rbSize,
        UE->mac_stats.ul.current_rbs);
  rk_slice_runtime_note_ul_grant(
      UE->rnti,
      sched_pusch->frame * nr_mac->frame_structure.numb_slots_frame + sched_pusch->slot
  );

  LOG_I(NR_MAC,
        "RK-SLICE UL-LIMIT UE %04x ul_floor=%d ul_cap=%d ul_maxcg=%d rbSize=%d\n",
        UE->rnti,
        ul_pol_log->ul_rb_floor,
        ul_pol_log->ul_rb_cap,
        ul_pol_log->ul_max_consecutive_grants,
        sched_pusch->rbSize);
