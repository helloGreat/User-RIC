...
#include "/tmp/rk_ue_dl_policy.h"
#include "/tmp/rk_ue_dl_policy.c"

#define RK_UE_DL_STATE_PATH "/opt/oai-rk/ue_la/ue_la.conf"
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  const int dl_now_tick = frame * slots_per_frame + slot;
  rk_ue_dl_reload_if_needed(RK_UE_DL_STATE_PATH);
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      const uint16_t current_rnti = UE->rnti;
      const int max_mcs_table = current_BWP->mcsTableIdx == 1 ? 27 : 28;
      const int eff_dl_max_mcs = rk_ue_dl_get_max_mcs(current_rnti, max_mcs_table);
      const int max_mcs = min(sched_ctrl->dl_max_mcs, eff_dl_max_mcs);
      ...
      const int dl_slice = rk_slice_assoc_get_dl_id(current_rnti);
      const rk_slice_policy_t *dl_pol = rk_slice_policy_get(dl_slice);
      const float dl_mul = dl_pol->dl_weight_mul;
      const int dl_maxcg_override =
          rk_ue_dl_get_maxcg_override(current_rnti, -1);
      const int dl_maxcg =
          (dl_maxcg_override > 0) ? dl_maxcg_override : dl_pol->dl_max_consecutive_grants;
      const int dl_throttled =
          rk_slice_runtime_dl_should_throttle(current_rnti, dl_now_tick, dl_maxcg);

      const float dl_posture_mul = rk_ue_dl_get_sched_mul(current_rnti, 1.0f);
      const int dl_small_burst_bytes =
          rk_ue_dl_get_small_burst_bytes(current_rnti, 0);
      const float dl_small_burst_mul =
          rk_ue_dl_get_small_burst_mul(current_rnti, 1.0f);

      const int dl_small_burst_hit =
          (dl_small_burst_bytes > 0 &&
           sched_ctrl->num_total_bytes > 0 &&
           sched_ctrl->num_total_bytes <= dl_small_burst_bytes);

      const float coeff_after_slice = coeff_ue * dl_mul;
      const float coeff_after_burst =
          coeff_after_slice * (dl_small_burst_hit ? dl_small_burst_mul : 1.0f);
      const float final_coeff =
          dl_throttled ? 0.0f : (coeff_after_burst * dl_posture_mul);

      UE_sched[numUE].coef = final_coeff;

      LOG_I(NR_MAC,
            "RK-SLICE DL-ORDER UE %04x dl_slice=%d dl_mul=%.2f coeff=%.4f final=%.4f\n",
            current_rnti,
            dl_slice,
            dl_mul,
            coeff_ue,
      ...
      LOG_I(NR_MAC,
            "RK-LA DL-ORDER UE %04x dl_sched_mul=%.2f dl_small_burst_hit=%d dl_small_burst_mul=%.2f coeff_after_slice=%.4f coeff_after_burst=%.4f final=%.4f\n",
            current_rnti,
            dl_posture_mul,
            dl_small_burst_hit,
            dl_small_burst_mul,
      ...
      if (dl_throttled) {
        LOG_I(NR_MAC,
              "RK-SLICE DL-LIMIT UE %04x dl_maxcg=%d throttled=1\n",
              current_rnti,
              dl_maxcg);
      }
  ...
  const int min_rbSize_default = 5;
  ...
  while (iterator->UE != NULL) {
    ...
    const int eff_dl_max_mcs = rk_ue_dl_get_max_mcs(rnti, 28);
    const int eff_dl_min_rb = rk_ue_dl_get_min_grant_prb(rnti, min_rbSize_default);
    const float eff_dl_sched_mul = rk_ue_dl_get_sched_mul(rnti, 1.0f);
    const int eff_dl_maxcg = rk_ue_dl_get_maxcg_override(rnti, -1);
    const int eff_dl_small_burst_bytes = rk_ue_dl_get_small_burst_bytes(rnti, 0);
    const float eff_dl_small_burst_mul = rk_ue_dl_get_small_burst_mul(rnti, 1.0f);

    LOG_I(NR_MAC,
          "RK-LA DL-CFG UE %04x dl_max_mcs=%d dl_min_grant_prb=%d dl_sched_mul=%.2f dl_maxcg_override=%d dl_small_burst_bytes=%d dl_small_burst_mul=%.2f\n",
          rnti,
          eff_dl_max_mcs,
          eff_dl_min_rb,
          eff_dl_sched_mul,
          eff_dl_maxcg,
          eff_dl_small_burst_bytes,
          eff_dl_small_burst_mul);
    ...
    if (remainUEs[beam.idx] == 0 || n_rb_sched[beam.idx] < eff_dl_min_rb) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    sched_pdsch->mcs = min(sched_pdsch->mcs, eff_dl_max_mcs);
    ...
    const int dl_slice_alloc = rk_slice_assoc_get_dl_id(iterator->UE->rnti);
    const rk_slice_policy_t *dl_pol_alloc = rk_slice_policy_get(dl_slice_alloc);
    const int dl_floor = dl_pol_alloc->dl_rb_floor;
    const int dl_cap = dl_pol_alloc->dl_rb_cap;

    if (dl_floor > 0 && rbSize < dl_floor) {
      const int target_floor = (dl_floor < max_rbSize) ? dl_floor : max_rbSize;
      if (target_floor > rbSize)
        rbSize = target_floor;
    }

    if (dl_cap > 0 && rbSize > dl_cap)
      rbSize = dl_cap;
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  UE_iterator(UE_info->connected_ue_list, UE) {
    ...
    if (harq->round != 0) { /* retransmission */
      ...
      const int dl_slice_log = rk_slice_assoc_get_dl_id(UE->rnti);
      const rk_slice_policy_t *dl_pol_log = rk_slice_policy_get(dl_slice_log);

      LOG_I(NR_MAC,
            "RK-SLICE DL-ALLOC UE %04x dl_slice=%d dl_cap=%d rbSize=%d current_rbs=%u\n",
            UE->rnti,
            dl_slice_log,
            dl_pol_log->dl_rb_cap,
            sched_pdsch->rbSize,
            UE->mac_stats.dl.current_rbs);
      rk_slice_runtime_note_dl_grant(
          UE->rnti,
          frame * gNB_mac->frame_structure.numb_slots_frame + slot
      );

      LOG_I(NR_MAC,
            "RK-SLICE DL-LIMIT UE %04x dl_floor=%d dl_cap=%d dl_maxcg=%d rbSize=%d\n",
            UE->rnti,
            dl_pol_log->dl_rb_floor,
            dl_pol_log->dl_rb_cap,
            rk_ue_dl_get_maxcg_override(UE->rnti, dl_pol_log->dl_max_consecutive_grants),
            sched_pdsch->rbSize);
